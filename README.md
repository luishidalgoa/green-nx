...
> **About this fork.** This is a feature fork maintained by
> **[@luishidalgoa](https://github.com/luishidalgoa)**, built on the original
> [**green-nx** by rmrf404](https://github.com/rmrf404/green-nx). It adds remote
> play from your own console (xHome), an on-screen debug HUD, audio-reliability
> fixes, a UI redesign, touch controls and other UX work. General fixes are
> contributed back upstream. All credit for the original client goes to rmrf404
> — see [Acknowledgements](#acknowledgements).

## What this fork adds over upstream

- **Remote play from your own Xbox (xHome)** — stream the console sitting in your
  living room over your own network, not just cloud games. Includes automatic
  wake + connection retries while the console registers.
- **On-screen debug HUD** — live **resolution, FPS, bitrate, packet loss and
  buffer** while you play, toggleable in Settings.
- **Audio-reliability fixes** — correct RTP demux so xHome audio is never
  silent, a jitter-buffer reset that kills the intermittent no-audio on
  reconnect, a soft-knee output limiter, and an adjustable output volume.
- **"OLED premium" UI redesign** and an on-screen version indicator.
- **Touchscreen navigation** — tap and swipe the menus, with a tappable hint bar.
- **Menu navigation sounds** and **friendlier, word-wrapped error messages**.
- **"1080p low-latency" quality option** — presents frames the instant they
  decode (less lag) alongside the standard tiers.
...
> **Release lines:** stable builds are tagged on `main`; the latest integrated
> work (including the debug HUD and newer features) lives on `develop`. See
> [`docs/BRANCHES.md`](docs/BRANCHES.md) for the full branch map.
...
| Settings | Stream quality (720p / 1080p / 1080p low-latency) · button layout · vibration · volume · **Debug HUD** on/off · preferred source (xCloud / your Xbox) |
